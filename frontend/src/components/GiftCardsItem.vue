<template>
  <section class="panel panel-brand">
    <table class="table table-hover table-striped">
      ...
      <tbody>
        <tr
          v-for="(card, index) in giftCardsList"
          :key="card.accountSummaryGiftCard.accountKey"
        >
          ...
          <td v-if="depositResponse.balance">
            {{ formatNumber(depositResponse.balance / 100) }}
          </td>
          <td>
            <div>
              <Button class="btn btn-primary" @click="buttonNumber(index)"
                >Predeposit</Button
              >
            </div>
          </td>
